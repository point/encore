  ...
  (if-let [ks-seq (seq ks)]
    (let [k (nth ks 0)]
      (if-let [ks (next ks-seq)]
        ...
        (if (kw-identical? f :update/dissoc)
          (dissoc m k)
          (let [v (f (get m k))]
            (if (kw-identical? v :update/dissoc)
              (dissoc m k)
              (assoc  m k v))))))
            ...
            (fn rf2 [acc k rv]
              (let [lv (get acc k not-found)]
                (cond
                  ...
                  :else
                  (let [new-rv (f lv rv)]
                    (if (kw-identical? new-rv :update/dissoc)
                      (dissoc acc k)
                      (assoc  acc k new-rv))))))
            (or acc {})
            in)))
...
;;; TODO
;; - (defn wrapf [old f] (let [new (f old)] (Swapped. new [old new])))

(do
  (deftype Swapped [new return])
  #+clj  (defn          swapped? [x] (instance? Swapped x))
  #+cljs (defn ^boolean swapped? [x] (instance? Swapped x))
  (defn -swapped ^Swapped [x] (if (instance? Swapped x) x (Swapped. x x)))
  (defn  swapped ^Swapped [new-val return-val] (Swapped. new-val return-val))
  (defn -swapped-vec [x]
    (if (instance? Swapped x)
      [(.-new ^Swapped x) (.-return ^Swapped x)]
      [x x]))

  (comment (qb 1e6 (.-new (-swapped "foo")))))
...
(defn swap-in!
  "Like `swap!` but supports `update-in` semantics, arbitrary return vals
  via `swapped`."
  ([atom_ f]
   (loop []
     (let [v0 @atom_
           s1 (-swapped (f v0))]
       (if (-cas! atom_ v0 (.-new s1))
         (.-return s1)
         (recur)))))
...
(defn reset-in [m & ops]
  (reduce
    (fn [m ?op]
      (if-not ?op
        m ; Support conditional ops
        (let [[ks val] ?op] (update-in* m ks (fn [_] val)))))
    {}
    ops))
...
(defn reset-in! "Is to `reset!` as `swap-in!` is to `swap!`."
  ([atom_    new-val] (reset!   atom_ new-val))
  ([atom_ ks new-val]
   (if (seq ks)
  ...
  ([atom_ ks new-val & more]
   (loop []
     (let [m0 @atom_
           m1 (reduce-kvs (fn [acc ks v]
                            (if (seq ks)
                              (assoc-in acc ks v)
                              v)) m0
                (cons ks (cons new-val more)))]
